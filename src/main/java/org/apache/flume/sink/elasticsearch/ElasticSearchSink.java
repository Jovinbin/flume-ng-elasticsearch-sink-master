...
import static org.apache.flume.sink.elasticsearch.FlumeEsCustomConstants.DEFAULT_INDEX_NAME_TIME;
...
public class ElasticSearchSink extends AbstractSink implements Configurable, BatchSizeSupported {
  ...
  public static int newIndexNameTime = DEFAULT_INDEX_NAME_TIME;
  ...
  public void configure(Context context) {
    ...
    //这一个是自己加的
    if (StringUtils.isNotBlank(context.getString("newIndexNameTime"))){
      newIndexNameTime = Integer.parseInt(context.getString("newIndexNameTime")) * 1000;
    }
    logger.info("间隔时间:{}", newIndexNameTime);
    //加入定时任务 每秒钟就去新建一个索引 避免单个索引库数据量过大
    Timer timer = new Timer();
    timer.schedule(new TimerTask() {
      @Override
      public void run() {
        nowTime = sdf.format(new Date());
        logger.info("生成一个新的索引时间:{}", nowTime);
      }
    }, 0, newIndexNameTime);
    //自带的
    if (!isLocal) {
  ...
  public void start() {
    ...
    logger.info("eventSerializer:{}",eventSerializer);
    logger.info("indexRequestFactory:{}",indexRequestFactory);
    sinkCounter.start();
    try {
      if (isLocal) {
